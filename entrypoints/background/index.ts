...
async function processCloudChange(cloudRaw: unknown): Promise<void> {
  if (!isSyncEnvelopeV1(cloudRaw)) {
    debugLog('ignored non-v1 cloud sync data')
    return
  }
  ...
  // Rule 5: Same version from a different device means conflict.
...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener(async (message) => {
    ...
    } else if (message.type === 'SYNC_LOCAL_CHANGED') {
      if (!isInited) return
