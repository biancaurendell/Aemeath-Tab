...
const searchSizeStyle = computed(() => {
  const size = Number.isFinite(settings.search.size) ? settings.search.size : 100
  const scale = Math.min(1.3, Math.max(0.8, size / 100))

  return {
    '--search-size-scale': scale,
    '--width': settings.perf.searchBar.launchAnim ? (mounted.value ? undefined : '0') : undefined,
  }
})
  ...
  <section ref="searchBox" class="search-box">
    ...
    >
      ...
      <input
        ...
        @keydown.enter.ctrl.prevent="doSearchWithText(searchText, true)"
        @keydown.enter.meta.prevent="doSearchWithText(searchText, true)"
