...
// 根据屏幕宽度初始区块的可见项目：所有手动快捷方式都进 Dock，再按置顶状态分区。
const dockShortcuts = computed(() =>
  shortcuts.value.map((item, originalIndex) => ({
    ...item,
...
const pinnedShortcuts = computed(() => dockShortcuts.value.filter((item) => item.isPinned))
const unpinnedShortcuts = computed(() => dockShortcuts.value.filter((item) => !item.isPinned))
const visiblePinnedShortcuts = computed(() => pinnedShortcuts.value.slice(0, maxFitCols.value))
const visibleUnpinnedShortcuts = computed(() =>
  unpinnedShortcuts.value.slice(0, Math.max(0, maxFitCols.value - visiblePinnedShortcuts.value.length)),
)
const visibleShortcutCount = computed(
  () => visiblePinnedShortcuts.value.length + visibleUnpinnedShortcuts.value.length,
)
const visibleTopSites = computed(() =>
  topSites.value.slice(0, Math.max(0, maxFitCols.value - visibleShortcutCount.value)),
)
...
function onItemContextmenu(
  ...
  item: { url: string; title?: string; favicon?: string },
  isPinned: boolean,
  isStored: boolean,
...
<template>
  ...
  >
    ...
    <template v-for="(item, idx) in visiblePinnedShortcuts" :key="`pin-${item.originalIndex}`">
      <el-tooltip
        :content="item.title"
        placement="top"
        effect="light"
      ...
      >
        <OnLongPress
          ...
          @contextmenu.stop.prevent="onItemContextmenu($event, item, true, true, item.originalIndex)"
          @trigger="onItemLongPress($event, item, true, true, item.originalIndex)"
        >
          <img :src="item.favicon || getOrCreateFaviconRef(item.url)" alt="favicon" />
        </OnLongPress>
      </el-tooltip>
      <div
        v-if="idx !== visiblePinnedShortcuts.length - 1"
        class="dock-gap"
        :ref="setScalableRef"
      ></div>
    </template>
    <template v-if="visiblePinnedShortcuts.length > 0 && visibleUnpinnedShortcuts.length > 0">
      <div class="dock-gap" :ref="setScalableRef"></div>
      <div class="dock-separator"></div>
      <div class="dock-gap" :ref="setScalableRef"></div>
    </template>
    <template v-for="(item, idx) in visibleUnpinnedShortcuts" :key="`shortcut-${item.originalIndex}`">
      <el-tooltip
        :content="item.title"
        placement="top"
        effect="light"
      ...
      >
        <OnLongPress
          ...
          :ref="setScalableRef"
          :target="settings.dock.openInNewTab ? '_blank' : '_self'"
          @contextmenu.stop.prevent="onItemContextmenu($event, item, false, true, item.originalIndex)"
          @trigger="onItemLongPress($event, item, false, true, item.originalIndex)"
        >
          <img :src="item.favicon || getOrCreateFaviconRef(item.url)" alt="favicon" />
        </OnLongPress>
      </el-tooltip>
      <div
        v-if="idx !== visibleUnpinnedShortcuts.length - 1"
        class="dock-gap"
        :ref="setScalableRef"
      ></div>
    </template>
    <template v-if="visibleShortcutCount > 0 && visibleTopSites.length > 0">
      <div class="dock-gap" :ref="setScalableRef"></div>
      <div class="dock-separator"></div>
      <div class="dock-gap" :ref="setScalableRef"></div>
    </template>
    <template v-for="(item, j) in visibleTopSites" :key="`top-${j}`">
      ...
      >
        <OnLongPress
          ...
          :ref="setScalableRef"
          :target="settings.dock.openInNewTab ? '_blank' : '_self'"
          @contextmenu.stop.prevent="onItemContextmenu($event, item, false, false, j)"
          @trigger="onItemLongPress($event, item, false, false, j)"
        >
          <img :src="item.favicon || getOrCreateFaviconRef(item.url)" alt="favicon" />
        </OnLongPress>
      </el-tooltip>
