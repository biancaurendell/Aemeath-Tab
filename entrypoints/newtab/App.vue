...
import AemeathLayer from './components/AemeathLayer.vue'
import Background from './components/Background.vue'
import Clock from './components/Clock.vue'
import DailyBoard from './components/DailyBoard.vue'
import QuickCommandCenter from './components/QuickCommandCenter.vue'
...
import KeyCommand20Filled from '~icons/fluent/key-command-20-filled'
...
const QuickCommandCenterRef = ref<InstanceType<typeof QuickCommandCenter>>()
const DailyBoardRef = ref<InstanceType<typeof DailyBoard>>()
...
const { conflictDialogVisible, conflictPayload } = storeToRefs(syncStore)
...
function focusDailyBoard() {
  DailyBoardRef.value?.focus()
}

function startDailyTimer() {
  DailyBoardRef.value?.startFocusTimer()
}

function openSettingsPage() {
  SettingsPageRef.value?.toggle()
}

function openBackgroundPreference() {
  BGSwticherRef.value?.show()
}

function openSearchEnginePreference() {
  SESwitcherRef.value?.show()
}

function openAddShortcut() {
  AddShortcutDialogRef.value?.openAddDialog()
}

function openQuickCommandCenter() {
  QuickCommandCenterRef.value?.toggle()
}
</script>
...
<template>
  ...
  >
    ...
    >
      ...
      <search-box v-if="settings.search.enabled" ref="SearchBoxRef" @contextmenu.stop />
      <shortcut
    ...
    <daily-board v-if="settings.aemeath.dailyBoard.enabled" ref="DailyBoardRef" />
    <div class="action-btn-container" :class="actionClass">
      <div
        role="button"
        tabindex="0"
        class="action-btn command-btn"
        :aria-label="$t('newtab:quickCommand.title')"
        :title="$t('newtab:quickCommand.hint')"
        @click="openQuickCommandCenter"
        @keydown.enter="openQuickCommandCenter"
      >
        <el-icon><key-command20-filled /></el-icon>
      </div>
      <settings-btn
    ...
    <quick-command-center
      ref="QuickCommandCenterRef"
      :on-focus-search="focusSearchBox"
      :on-focus-daily-board="focusDailyBoard"
      :on-start-daily-timer="startDailyTimer"
      :on-open-settings="openSettingsPage"
      :on-open-background-switcher="openBackgroundPreference"
      :on-open-search-engine-preference="openSearchEnginePreference"
      :on-open-bookmark-sidebar="showBookmarkSidebar"
      :on-open-add-shortcut="openAddShortcut"
    />
