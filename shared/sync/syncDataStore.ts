...
import { CURRENT_CONFIG_VERSION, defaultSettings, useSettingsStore } from '../settings'
  ...
  const applyCloudData = async (cloudInput: SyncEnvelopeV1) => {
    ...
    try {
      ...
      if (!isSyncEnvelopeV1(cloudData)) {
        ...
        throw new Error('Unsupported cloud sync data format.')
        return
      }
      const shortcutStore = useShortcutStore()
      const customSearchEngineStore = useCustomSearchEngineStore()
      ...
      if (cloudData.configVersion < CURRENT_CONFIG_VERSION) {
        localSettings.sync.enabled = false
        throw new Error(`Unsupported cloud config version: ${cloudData.configVersion}`)
      }

      const localState = structuredClone(localSettings.getRawState())
      const mergedSettings = restoreDeviceLocalFields(cloudData.settings, localState)
