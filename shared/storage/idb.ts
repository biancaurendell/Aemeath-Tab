...
interface AemeathDBSchema extends DBSchema {
  favicon: {
    key: string
...
let dbPromise: Promise<import('idb').IDBPDatabase<AemeathDBSchema>> | null = null
...
function getDB() {
  if (!dbPromise) {
    dbPromise = (async () => {
      ...
      return openDB<AemeathDBSchema>(DB_NAME, targetVersion, {
        upgrade(db) {
...
export async function idbGet<S extends StoreName>(
  storeName: S,
  key: string,
): Promise<AemeathDBSchema[S]['value'] | undefined> {
